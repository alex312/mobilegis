"use strict";
var __decorate = (this && this.__decorate) || function (decorators, target, key, desc) {
    var c = arguments.length, r = c < 3 ? target : desc === null ? desc = Object.getOwnPropertyDescriptor(target, key) : desc, d;
    if (typeof Reflect === "object" && typeof Reflect.decorate === "function") r = Reflect.decorate(decorators, target, key, desc);
    else for (var i = decorators.length - 1; i >= 0; i--) if (d = decorators[i]) r = (c < 3 ? d(r) : c > 3 ? d(target, key, r) : d(target, key)) || r;
    return c > 3 && r && Object.defineProperty(target, key, r), r;
};
var __metadata = (this && this.__metadata) || function (k, v) {
    if (typeof Reflect === "object" && typeof Reflect.metadata === "function") return Reflect.metadata(k, v);
};
var core_1 = require('@angular/core');
var feature_1 = require('../data/feature');
var ship_1 = require('../../ship');
var traffic_env_1 = require('../../traffic-env');
var FeatureInfoComponent = (function () {
    function FeatureInfoComponent() {
        this.FeatureType = feature_1.FeatureType;
    }
    __decorate([
        core_1.Input(), 
        __metadata('design:type', feature_1.Feature)
    ], FeatureInfoComponent.prototype, "FeatureData", void 0);
    FeatureInfoComponent = __decorate([
        core_1.Component({
            selector: 'feature-info',
            templateUrl: 'build/plugins/map/component/feature-info.component.html',
            directives: [ship_1.ShipInfoComponent, traffic_env_1.TrafficEnvInfoComponent]
        }), 
        __metadata('design:paramtypes', [])
    ], FeatureInfoComponent);
    return FeatureInfoComponent;
}());
exports.FeatureInfoComponent = FeatureInfoComponent;

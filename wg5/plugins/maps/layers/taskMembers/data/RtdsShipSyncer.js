...
define(["require", "exports", "long", "protobufjs", "./exprs", "./rtds", "text!./ships.proto"], function (require, exports, Long, ProtoBuf, exprs_1, rtds_1, proto) {
    "use strict";

    var PBMessages = ProtoBuf.loadProto(proto).build();

    var ShipSyncer = function () {
        function ShipSyncer(options) {
            _classCallCheck(this, ShipSyncer);

            this.items_ = {};
            this.local_ = options.local;
            this.remote_ = options.remote || "ws://" + /\/\/([^\/]+)\//.exec(document.URL)[1] + ":18899";
            this.bounds_ = null;
            this.watchees_ = {};
            this.filters_ = Object.create(ShipSyncer.DefaultFilters_);
            this.appliedFilters_ = Object.create(ShipSyncer.DefaultFilters_);
            if (isFinite(options.expires)) this.filters_.ttl = options.expires;
            this.connect_();
        ...
        _createClass(ShipSyncer, [{
            ...
            key: "watch",
            value: function watch(id, initial) {
            ...
            value: function unwatch(id) {
                ...
                if (w) {
                    if (! --w.refs) {
                        if (w.data) this.local_.remove(w.data);
            ...
            key: "expire_",
            value: function expire_() {
                console.log(this.appliedFilters_.ttl);
            ...
            key: "doRefilter_",
            value: function doRefilter_() {
                var watchees = Object.getOwnPropertyNames(this.watchees_);
                if (!areBoundsesEqual(this.appliedFilters_.bounds, this.bounds_) || !areSetEqual(this.appliedFilters_.watchees, watchees) || !areSetEqual(this.appliedFilters_.types, this.filters_.types) || !areSetEqual(this.appliedFilters_.signals, this.filters_.signals) || this.appliedFilters_.nationcat !== this.filters_.nationcat || areArrayEqual(this.appliedFilters_.sog, this.filters_.sog) || areArrayEqual(this.appliedFilters_.length, this.filters_.length) || this.appliedFilters_.ttl !== this.filters_.ttl) {
                    ...
                    if (this.client_) this.client_.setFilter(this.createRtdsFilter_());
                }
            }
        }, {
            key: "connect_",
            ...
            key: "onClientDataItem_",
            value: function onClientDataItem_(action, item) {
            ...
            key: "onClientOpen_",
            value: function onClientOpen_() {
                this.reset_();
                this.doRefilter_();
            }
        }, {
            key: "onClientClose_",
            value: function onClientClose_() {
                this.client_ = null;
            }
        }, {
            key: "onReconnectTimer_",
            value: function onReconnectTimer_() {
                if (!this.client_) {
            ...
            value: function reset_() {
                ...
                var items = [];
                for (var each in this.items_) {
                    if (!this.items_.hasOwnProperty(each)) continue;
            ...
            key: "createRtdsFilter_",
            value: function createRtdsFilter_() {
                //return exprs.Constant.bool(true);
                ...
                var unfiltered;
                if (!bounds && !watchees) unfiltered = false;else if (!bounds) unfiltered = watchees;else if (!watchees) unfiltered = bounds;else unfiltered = new exprs_1.default.Function("||", bounds, watchees);
                if (unfiltered === false) return exprs_1.default.Constant.bool(false);
                ...
                var f = [types, signals, sog, length, nationcat, ttl].reduce(function (prev, current) {
                    if (prev === false || current === false) return false;
                    if (prev === null) return current;
                    if (current === null) return prev;
                    return new exprs_1.default.Function("&&", prev, current);
                });
                if (f === false) return exprs_1.default.Constant.bool(false);
            ...
            set: function set(value) {
                if (this.filters_ !== value) {
                    if (this.filters_.un instanceof Function) this.filters_.un('propertychange', this.$refilter$_);
                    this.filters_ = value || ShipSyncer.DefaultFilters_;
                    if (this.filters_.on instanceof Function) this.filters_.on('propertychange', this.$refilter$_);
                    this.refilter_();
                }
            }
        }, {
    ...
    function createShipCargoTypesExpression(types) {
        ...
        for (var i = 0; i < types.length; i++) {
            ...
            if (mapped) ranges.push.apply(ranges, mapped);
        }
    ...
    function createRangesExpression(ranges, field, type) {
        ...
        for (var i = 0; i < ranges.length; i++) {
            ...
            if (r[1] !== Infinity) minmax.push(new exprs_1.default.Function("<", f, const_constructor(r[1])));
            if (minmax.length === 2) exprRanges.push(new exprs_1.default.Function("&&", minmax[0], minmax[1]));else if (minmax.length === 1) exprRanges.push(minmax[0]);
        }
        if (!exprRanges.length) return null;
    ...
    function convertItem(item) {
        ...
        if (!isNaN(ret.dynamicTime.getTime())) ret.time = ret.dynamicTime;else if (!isNaN(ret.staticTime.getTime())) ret.time = ret.staticTime;
        return ret;
    }
    function convertDimensions(length, width, prow, larboard) {
        if (length && width) return [prow, length - prow, larboard, width - larboard];
        return null;
    }
    function convertTime(t) {
        if (!t || t.isZero()) return new Date(NaN);

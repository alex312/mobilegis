...
class ShipsLayerShapeVts {
    ...
    constructor(option) {
        ...
        var sog = data.sog;
        var cog = data.cog;
        if (sog !== 102.3 && sog !== 0 && cog < 360) { //todo 102.3是什么
        ...
        var pointResolution = projection.getPointResolutionFunc(option.resolution, ol.proj.fromLonLat([data.lon, data.lat]));
        ...
        var zoom = MapTool.ResolutionToZoom(option.resolution);
    ...
    getColor = function (shipType, shipVType) {
        //var a = shipType % 10;
        //var b = (shipType - a) / 10;
    ...
    SHIPCOLORS = JSTool.ArrayIAToObject(StaticLib.ShipType.Names, StaticLib.ShipType.Colors)
...
ShipShape.addImplementation(ShipsLayerShapeVts);

export default ShipsLayerShapeVts;

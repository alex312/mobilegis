...
import {sprite} from "seecool/utilities";
import {MapTool} from 'seecool/utils/MapTool';
import POINTTYPES from 'seecool/utils/POINTTYPES';
import ShipsLayerLabelArranger from "./ShipsLayerLabelArranger";
import DouglasPeucker from "seecool/utils/DouglasPeucker";
import LabelRectManager from '../data/LabelRectManager';
import {ShipsLayerFeature} from "./ShipsCanvasSource";
var mapTool=MapTool;

class ShipsLayerDrawer {
    flagFactory;
    fontSize;
    iconSize;
    labelRectManager;
    ...
    constructor(shapeFactory, flagFactory) {
        ...
        this.labelRectManager = new LabelRectManager();
        this.flagRectManager = new LabelRectManager();
    }

    setParameters = function (extent, resolution, size, ships) {
        this.extent_ = extent;
    ...
    draw = function () {
        ...
        if (canvas.width !== this.size_[0])
            canvas.width = this.size_[0];
        if (canvas.height !== this.size_[1])
            canvas.height = this.size_[1];
    ...
    updateShips_ = function () {
        var ships = this.ships_;
        for (var each in ships) {
            var ship:ShipsLayerFeature = ships[each];
    ...
    updateShipShape_ = function (ship) {
        var data = ship.data;
        // console.log(data.time);
        var changed = !ship.shape || !data.time ||
    ...
    updateShipIcons_ = function (ship) {
        var icons = ship.icons = [];
        var data = ship.data;

        if (data.device)
            icons.push(sprite.get(data.device));
    };

    cullAndSortShips_ = function () {
        var ships = this.ships_;
        var result = [];
        for (var each in ships) {
            var ship = ships[each];
            if (ship.isFocused() || ship.isFollowed())
                result.push(ship);
            else if (ol.extent.containsCoordinate(this.extent_, ship.lonlat)) {
                result.push(ship);
            }
        }
    ...
    drawShips_ = function (context, ships) {
        var drawn = [];
    ...
    rarefyTrackPoints_ = function (feature) {
    ...
    getActualPoints_ = function (track, stopTime) {
        var stopIndex = 0;
        if (stopTime) {
            for (var i = 0; i < track.length; i++) {
                if (stopTime > track[i].time)
                    stopIndex++;
            }
        }
        ...
        if (stopIndex < actualPoints.length)  //todo:2016.04.07  补丁
            actualPoints[stopIndex].fixed = true;
        actualPoints = DouglasPeucker.byDistance(actualPoints, 5, 30);
        actualPoints[0].type = POINTTYPES.START;
        return {points: actualPoints, index: stopIndex};
    };

    convertToPoint_ = function (lonlat) {
    ...
    drawTrack_ = function (context, feature) {
        ...
        if (TrackPoints && TrackPoints.length > 0) {
            ...
            for (var i = 0; i < ps.length; i++) {
                ...
                if (this.resolution_ < 600 && TrackPoints[i].time && ship.time > TrackPoints[i].time) {
                    ...
                    //绘制航迹点标签
                    if (!isNaN(TrackPoints[i].time))
                        if (this.labelRectManager.tryPut({left: p1[0], top: p1[1], width: 80, height: 16})) {
                            ...
                            if (!heading || Math.round(heading) == 511)
                                heading = "?";
                            var t = ''
    ...
    drawFlag_ = function (context, list, xy) {
        if (list && list.length > 0) {
            ...
            var w = width * 2 + 2, h = 20, x = 0, y = 20, r = 5, u = 2;
    ...
    drawLabel_ = function (context, label/*, icons*/) {
        var line = label.line;
        if (!line)
            return;
        context.strokeStyle = "#333333";
        ...
        if (icons) {
            for (var i = 0, items = icons.value, x = icons.x; i < items.length; i++) {
                ...
                context.drawImage(icon.image,
                    frame.x, frame.y, frame.width, frame.height,
                    x, icons.y, this.iconSize, this.iconSize);
                x += this.iconSize;
            }
        }
    };

    arrangeLabels_ = function (context, ships) {
        ...
        var labelArranger = new ShipsLayerLabelArranger(25, this.resolution_ < 1.194328566955879 ? 200 : 25, 25);
...
export default ShipsLayerDrawer;

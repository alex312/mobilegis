    ...
    (function (rtds) {
        var RtdsClient = function () {
            function RtdsClient(server) {
                ...
                _classCallCheck(this, RtdsClient);
                ...
                var protocol = this.protocol_ = new RtdsProtocol(function (data) {
                    if (sock.readyState === WebSocket.OPEN) sock.send(data);else me.pendingOutData_.push(data);
                });
            ...
            _createClass(RtdsClient, [{
                ...
                key: "addParser",
                value: function addParser(name, pb) {
                ...
                value: function on(type, handler) {
                    if (this.events_.hasOwnProperty(type)) this.events_[type].push(handler);else this.events_[type] = [handler];
                }
            }, {
                key: "un",
                value: function un(type, handler) {
                ...
                key: "raiseEvent",
                value: function raiseEvent(type) {
                    if (this.events_.hasOwnProperty(type)) {
                        var handlers = this.events_[type];
        ...
        var RtdsProtocol = function () {
            function RtdsProtocol(sender) {
                _classCallCheck(this, RtdsProtocol);
            ...
            _createClass(RtdsProtocol, [{
                ...
                key: "addParser",
                value: function addParser(type, parser) {
                    this._parsers[type] = parser;
                }
            }, {
                key: "on",
                value: function on(eventType, handler) {
                    if (!(eventType in this._eventHandlers)) this._eventHandlers[eventType] = [handler];else this._eventHandlers[eventType].push(handler);
                }
            }, {
                key: "un",
                ...
                key: "onDataImteMessage",
                value: function onDataImteMessage(msg) {
                    ...
                    var id;
                    var payload;
                    if (action === 1 || action === 2) type = msg.readUTF8String(msg.readUint16(), ByteBuffer.METRICS_BYTES);
                    ...
                    if (action === 3) {
                        ...
                        var _item = this._parsers[type].decode(payload);
                        if (id in this._items) {
                            this._items[_item["Id"]] = _item;
                            this.fireItemEvent("update", _item);
                        } else {
                            this._items[_item["Id"]] = _item;
                            this.fireItemEvent("add", _item);
                        }
                    }
                ...
                key: "fireItemEvent",
                value: function fireItemEvent(action, item) {
        ...
        var MessageBuffer = function () {
            ...
            _createClass(MessageBuffer, [{
                ...
                value: function write(data) {
                    this.append(data);
                    var offset = 0,
                        remains = this._buffer.offset;
                    while (true) {
                        if (remains < 2) break;
                        var size = this._buffer.readUint16(offset);
                        if (size === 0) break;
                        if (remains < size) break;
                ...
                value: function append(data) {
                    ...
                    if (s > this._buffer.limit) {
                        var x;
                        if (this._buffer.limit) x = Math.floor((s + 4095) / this._buffer.limit) * this._buffer.limit;else x = 4096;

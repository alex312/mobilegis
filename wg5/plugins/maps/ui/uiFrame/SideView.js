    ...
    var SideView = function () {
        ...
        _createClass(SideView, [{
            ...
            value: function open(view) {
                var i = 0;
                for (; i < this.stack_.length; i++) {
                    if (this.stack_[i][0] === view) break;
                }
                if (i < this.stack_.length - 1) {
                    this.pop(-i + 1);
                } else if (i === this.stack_.length) {
            ...
            key: "close",
            value: function close(view) {
                for (var i = 0; i < this.stack_.length; i++) {
                    if (this.stack_[i][0] === view) {
                        this.pop(-i);
                        break;
                    }
                }
            ...
            value: function push(view) {
                if (this.stack_.length) {
                    ...
                    cv.element.style.opacity = '0';
                    setTimeout(function () {
                        if (entry[1] == mark) cv.element.style.visibility = 'hidden';
                    }, 300);
                }

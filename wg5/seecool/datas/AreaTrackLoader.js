...
define(["require", "exports", "jquery", "fecha", 'seecool/utilities'], function (require, exports, $, fecha, utilities) {
    ...
    var AreaTrackLoader = function () {
        function AreaTrackLoader(url, area, start, end, fnLoad, fnError, context, token) {
            _classCallCheck(this, AreaTrackLoader);
        ...
        _createClass(AreaTrackLoader, [{
            key: "load",
            value: function load() {
                console.log("area track loader loading");
                if (this.ajax_) return;
                ...
                if (queryEndTime > this.end_) queryEndTime = this.end_;
                ...
                var url = this.url + "/ShipRegion/ShipRegionUnionQuery/Rectangle(" + this.area_.join(",") + ")/" + fecha.format(queryStartTime, 'YYYY-MM-DD HH:mm:ss') + "/" + fecha.format(queryEndTime, 'YYYY-MM-DD HH:mm:ss');
                this.ajax_ = $.ajax({
                    url: url,
                    dataType: 'json',
                    type: 'GET',
                    ...
                    for (var j = 0; j < result.length; j++) {
                        ...
                        for (var i = 0; i < result[j].ShipTrackSignals.length; i++) {
                            ...
                            if (!arr) {
                                ...
                                var arr = this.undeterminedPoints_[uid];
                                if (!arr) this.undeterminedPoints_[uid] = [pt];else arr.push(pt);
                            }
                        }
                    }
                }

...
export function uniqueId() {
    _uniqueId_prefix = _uniqueId_prefix ||
        randomString_();
    _uniqueId_serial = _uniqueId_serial || 1;
    return _uniqueId_prefix +
        "_" + new Date().getTime() +
        "_" + _uniqueId_serial++;
}

export function dateFromWcfJson(str) {
    var reg = /(\d+)((\+|\-)(\d{2})(\d{2}))?/;
    var match: any = reg.exec(str);
    if (!match)
        return null;
...
export function isPhone(aPhone) {
    return /^(0|86|17951)?(13[0-9]|15[012356789]|18[0-9]|14[57])[0-9]{8}$/.test(aPhone);
}

export function parseIsoTimeSpan(str) {
    var match: any = /(P(\d+D)?)(T(\d+H)?(\d+M)?(\d+S)?)?/.exec(str);
...
export function formatTimeSpan(val) {
    if (!val)
        return "0秒";
    if (val < 1000)
        return "<1秒";
...
export var _uniqueId_prefix; // undefined
export var _uniqueId_serial; // undefined

export function getFormatDataFromURL() {
...
export function addLetterToString(str, len, letter, isBefore) {
    str = str.toString();
...
// L代表经度，B代表纬度
export function formatDegree(deg, style) {
    ...
    var m: any = Math.round((diff - c) * 60 * 100) / 100;
 ...
 */
export function box(context, rect, strokestyle, linewidth) {
...
export function leftUpRoundRect(context, x, y, w, h, radius, delX, delY) {
    var r = x + w;
    var b = y + h;
    context.beginPath();
...
export function leftRoundRect(context, x, y, w, h, radius, delta) {
    var r = x + w;
    var b = y + h;
    context.beginPath();
    context.moveTo(x + radius, y);
...
// 通用的hover显示框
export function hover(context, x, y, name, items, params) {
    if (items.length < 1) {
        return;
    }
...
// 依据mmsi获取国家名称
export function getMmsiNation(mmsi) {
    mmsi = "" + mmsi;
    if (!/^\d{9}$/.test(mmsi))
        return null;
...
export class RepeatingAjax {
    ...
    onAjaxFail(jqXHR, textStatus, errorThrown) {
        this._ajax = null;
...
export function fromWcfJson(str) {
    var reg = /(\d+)((\+|\-)(\d{2})(\d{2}))?/;
    var match: any = reg.exec(str);
    if (!match)
        return null;
...
export function getIconBaseUrl(type) {
    return "api/icon/images?type=" + type + "&id=";
}

export function scremoveAll(arr, item) {
    var ind = 0;
    while (true) {
...
export function Polygon(pointArray) {
...
export function Circular(radius): void {
    var _radius = radius;
    if (_radius < 0)
        _radius = 0;
    var _box = null;
...
export function degreeToDecimal(lonlat) {
    var lonlats = /(\d+°)?(\d+')?(\d+(\.\d+)?")?/.exec(lonlat);
    var deg: any = (lonlats[1] === undefined) ? "0°" : lonlats[1];
    var minute: any = (lonlats[2] === undefined) ? "0'" : lonlats[2];
    var second: any = (lonlats[3] === undefined) ? '0"' : lonlats[3];
...
export function getAisShipTypeName(code) {
    var name = ais_ship_types_[code];
    return name || '--';
}

export function getAisNavStatusName(code) {
...
export function globalTooltip(msg) {
...
export var notificationCenter = new NotificationCenter();
...
export namespace DC {
    ...
    // //>a={b:{c:[{d:{e:"e1"}}]}}
    // //>v={e:"e1"}
    export function V(obj, path, fun?) {
    ...
    export function SV(a, path, value) {
...
export function lon3857(lon: number) {
    if (isFinite(lon)) {
        let size = 262144 * 0.5971642833948135 * 256;
...
export function lat3857(lat: number) {
